...
CmdExec* CmdParser::parseCmd(string& option)
{
	...
	string cmd;
	myStrGetTok(str, cmd);
	CmdExec* temp = getCmd(cmd);
	if(temp == 0){
		cerr << "Illegal command!! (" << cmd << ")";
		return NULL;
	}
	size_t count = cmd.size();
	for(; count < str.size(); count++){
		if(str[count] != ' ')	break;
	}

	option = str.substr(count);
...
{
	// TODO...
	int n = (int)(_readBufPtr - _readBuf);
	string s = str.substr(0, n);
	vector<string> toks;
	string token;
	{
		size_t temp = myStrGetTok(s, token);
		while (token.size()) {
			toks.push_back(token);
			temp = myStrGetTok(s, token, temp);
		}
	}
	int size = toks.size();
	if (size == 0){
		...
		if (str[str.size() - 1] == ' ') {
			for (CmdMap::const_iterator it = _cmdMap.begin(); it != _cmdMap.end(); ++it) {
				if (myStrNCmp(it->first + it->second->getOptCmd(), toks[0], it->first.size()) == 0)
					cmds.push_back(it->first + it->second->getOptCmd());
			}
			if (cmds.size() == 1) {
				cout << endl;
				getCmd(cmds[0])->usage(cout);
				reprintCmd();
			} else	mybeep();
		}
		else {
			...
			if (cmdsSize == 0)	mybeep();
			if (cmdsSize == 1){
				for (size_t i = toks[0].size(); i < cmds[0].size(); ++i) {
					insertChar(cmds[0][i]);
				} insertChar(' ');
			}
			else { // cmdsSize > 1
				cout << endl;
				for (size_t i = 0, count = 0; i < cmdsSize; ++i) {
					cout << setw(12) << left << cmds[i];
					++ count;
					if(count == 5){
						count = 0;
						cout << endl;
...
CmdExec* CmdParser::getCmd(string cmd)
{
	...
	// TODO...
	for(CmdMap::const_iterator it = _cmdMap.begin(); it != _cmdMap.end(); it++){
		if(myStrNCmp(it->first + it->second->getOptCmd(), cmd, it->first.size()) == 0)	return it->second;
	}
