...
CmdExecStatus DBAppendCmd::exec(const string& option)
{
	...
	if(myStrNCmp("-Row", toks[0], 2) == 0){
		if (size == 1)	return CmdExec::errorOption(CMD_OPT_MISSING, toks[1]);
		size_t i = 1, nCols = dbtbl.nCols();
		vector<int> ints;
		for(; i < size && i <= nCols; i++){
			int num;
			if(toks[i] == "-")	ints.push_back(INT_MAX);
			else if(! myStr2Int(toks[i], num)){
				cerr << "Error: " << toks[i] << " is not a number!!\n";
				return CMD_EXEC_ERROR;
			}else	ints.push_back(num);
		}
		for(; i <= nCols; i++)
			ints.push_back(INT_MAX);
		
		dbtbl.addRow(DBRow(ints));
	}
	else if(myStrNCmp("-Column", toks[0], 2) == 0){
		if (size == 1)	return CmdExec::errorOption(CMD_OPT_MISSING, toks[1]);
		size_t i = 1, nRows = dbtbl.nRows();
		vector<int> ints;
		for(; i < size && i <= nRows; i++){
			int num;
			if(toks[i] == "-")	ints.push_back(INT_MAX);
			else if(! myStr2Int(toks[i], num)){
				cerr << "Error: " << toks[i] << " is not a number!!\n";
		...
		return CmdExec::errorOption(CMD_OPT_ILLEGAL, toks[0]);
...
//            | -Row (int rowIdx) | -Column (int colIdx) | -Table | -Summary>
//----------------------------------------------------------------------
...
{  
	...
	int size = toks.size(), num;
	if (size > 2)
		return CmdExec::errorOption(CMD_OPT_EXTRA, toks[2]);
	if (myStrNCmp("-Table", toks[0], 2) == 0) {
		if (size == 2)	return CmdExec::errorOption(CMD_OPT_EXTRA, toks[1]);
		cout << dbtbl;
	}
	else if (myStrNCmp("-Summary", toks[0], 2) == 0) {
		if (size == 2)	return CmdExec::errorOption(CMD_OPT_EXTRA, toks[1]);
		dbtbl.printSummary();
	}
	else if (myStrNCmp("-Row", toks[0], 2) == 0) {
		if (size == 1)	return CmdExec::errorOption(CMD_OPT_MISSING, toks[0]);
		if (checkRowIdx(toks[1], num))	cout << dbtbl[num];
		else	return CMD_EXEC_ERROR;
	}
	else if (myStrNCmp("-Column", toks[0], 2) == 0) {
		if (size == 1)	return CmdExec::errorOption(CMD_OPT_MISSING, toks[0]);
		if (checkRowIdx(toks[1], num))	dbtbl.printCol(num);
		else	return CMD_EXEC_ERROR;
	}
	else if (checkRowIdx(toks[0], num)) {
		int num2;
		if (size == 1)	return CmdExec::errorOption(CMD_OPT_MISSING, toks[0]);
		if (!myStr2Int(toks[1], num2))	return CmdExec::errorOption(CMD_OPT_ILLEGAL, toks[1]);
		if (checkColIdx(toks[1], num2)){
			int a = dbtbl.getData(num, num2);
			if(a == INT_MAX)	cout << "null\n";
			else				cout << a << endl;
		}
		else	return CMD_EXEC_ERROR;
	}
...
void DBPrintCmd::usage(ostream& os) const
{
	os << "DBPrint < (int rowIdx) (int colIdx)\n"
		<< "        | -Row (int rowIdx) | -Column (int colIdx) | -Table | -Summary>" << endl;
}
