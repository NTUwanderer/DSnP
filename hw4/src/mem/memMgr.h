...
// e.g. Let SIZE_T = 8;  oSizeT(7) = 8, toSizeT(12) = 16
...
class MemRecycleList
{
	...
	T* popFront() {
		...
		if(_first != 0){
			T* temp = _first;
			_first = *(reinterpret_cast<T **>(_first));
			return temp;
		}
		return _first;
	}
...
#define S sizeof(T)
	...
	public:
		...
		void reset(size_t b = 0) {
			...
			_activeBlock->reset(); // _ptr = _begin;
	...
	private:
		...
		T* getMem(size_t t) {
			...
			assert(t % SIZE_T == 0);
			if (t > _blockSize)	{
				cout << "Requested memory (" << t << ") is greater than block size(" << _blockSize
					<< "). Exception raised...\n";
			...
			if (ret != 0) {
				...
				if (!_activeBlock->getMem(t, ret)) {
					size_t rSize = _activeBlock->getRemainSize();
					if (rSize >= S) {
						size_t rn = getArraySize(rSize);
						_activeBlock->getMem(rSize, ret);
#ifdef MEM_DEBUG
